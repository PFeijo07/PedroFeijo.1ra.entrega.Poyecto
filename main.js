...
function cotizar(){
    ...
    switch(tipo_libro){
        case "1":
        let add = productos[0].precio * cantidad_libro
        carrito.push(add);
        break;
        case "2":
...
cotizar();
let salir= prompt("Quiere seguir comprando? si/no");
let sum=0;

if (salir="si"){ 
    
    for (let suma = 0; suma < array.length; i++) {
        sum += array[suma];

    }

    }else  cotizar();
    salir= prompt("Quiere seguir comprando? si/no");
